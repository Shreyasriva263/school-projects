...
int main(int argc, char **argv) {
    ...
    while (fgets(cmd, CMD_LEN, stdin) != NULL) {
        ...
        if (strcmp(first_token, "pwd") == 0) {
            ...
            char *cwd;    // current working directory pointer.
            char buffer[CMD_LEN];
            cwd = getcwd(buffer, CMD_LEN);
            if (cwd != 0) {
                printf("%s\n", buffer);
            } else {
                perror("getcwd");
        ...
        // Task 6: Wait for all background jobs
            ...
            if (pid == 0) {    // Child process
                ...
                if (!background) {    // Foreground process
                    ...
                    // 2) restore shell process as foreground group
                    if (tcsetpgrp(STDIN_FILENO, getpid()) == -1) {
                        perror("tcsetpgrp");
                    }

                    // 3) child stopped, add to job list
